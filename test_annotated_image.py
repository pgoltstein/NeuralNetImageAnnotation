...
anim2 = ia.AnnotatedImage(downsample=None)
...
annotations_grid,shift_grid = ia.image_grid_RGB( annotations,
    ...
    image_size=im_size, n_x=4, n_y=4, channel_order=ch_ord,
...
with sns.axes_style("white"):
    ...
    ax = list(range(anim1.n_channels))
    for ch in range(anim1.n_channels):
...
with sns.axes_style("white"):
    plt.figure(figsize=(12,8), facecolor='w', edgecolor='w')
    ax = list(range(anim2.n_channels))
    for ch in range(anim2.n_channels):
        ax[ch] = plt.subplot2grid( (2,3), (0,ch) )
